...
http://www.codeskulptor.org/#user40_tpof2AFr2M_13.py
"""
...
def greedy_boss(days_in_simulation, bribe_cost_increment, plot_type = STANDARD):
    ...
    curr_day = 0
    curr_sal = float(INITIAL_SALARY)
    tot_sal = 0
    balance = 0
    curr_bribe = INITIAL_BRIBE_COST
    ...
    while curr_day < days_in_simulation:
        ...
        if balance > curr_bribe:
            incr_day = 0
        else:
            incr_day = int(math.ceil((curr_bribe-balance)/curr_sal))
        # advance current_day to day of next bribe (DO NOT INCREMENT BY ONE DAY)
        curr_day += incr_day
        # update state of simulation to reflect bribe
        balance += (curr_sal * incr_day)
        balance -= curr_bribe
        tot_sal += (curr_sal * incr_day)
        curr_sal += SALARY_INCREMENT
        curr_bribe += bribe_cost_increment

        # update list with days vs total salary earned for most recent bribe
        # use plot_type to control whether regular or log/log plot
        if plot_type == STANDARD:
            days_vs_earnings.append((curr_day,int(tot_sal)))
        else:
            days_vs_earnings.append((math.log(curr_day),math.log(int(tot_sal))))
